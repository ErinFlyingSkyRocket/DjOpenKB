...
if looks_like_full_bootstrap:
    ...
    if missing_protected:
        if out and out[-1].strip():
            out.append("")
        out.append("# Protected service-account identities and credentials. Fill only the services you enable.")
        for key in missing_protected:
            if key == "SMTP_RELAY_PASSWORD_USE_LDAP_BIND_PASSWORD":
...
if added_protected:
    print("Added protected service-account placeholders: " + ", ".join(added_protected))
...
print("Manual values preserved/not generated: AI API keys, LDAP_BIND_DN, LDAP_BIND_PASSWORD, SMTP_RELAY_USERNAME, SMTP_RELAY_PASSWORD, SMTP_FROM_EMAIL")
print("DJANGO_SECRET_KEY and DJANGO_FIELD_ENCRYPTION_KEY are generated by separate random calls for fresh deployments.")
print()
