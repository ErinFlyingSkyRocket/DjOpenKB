...
The application uses a per-response CSP nonce generated by Django. The policy allows static assets from the same origin and only the nonce-bearing inline template blocks that need server-rendered values. `script-src` and `style-src` do not use `unsafe-inline`; inline event attributes and inline `style=` attributes are forbidden.

When changing templates, use external static JavaScript for click/submit handlers and CSS classes from static stylesheets instead of `onclick`, `onsubmit`, or `style=` attributes. For a new server-rendered inline `<script>` or `<style>` block, add `nonce="{{ csp_nonce }}"`. Do not configure a second CSP header in Nginx because browsers enforce multiple CSP headers together.
