...
### 9.2 Manual Existing-Article Editing and Private Draft Contexts
...
A normal writer editing a published article has two separate states. **Save draft** writes the proposed content to `pending_update_*` with `UpdateStatus.NONE`, which is private to the article owner and excluded from Manage Pending/reviewer editing. **Submit update for approval** changes the shared update state to `PENDING`. If an already-submitted or rejected update is saved as draft again, it is retracted to `NONE`, its submission/review timestamps are cleared, and it leaves the reviewer queue until explicitly resubmitted.

Managers and Admin Users editing a published article outside the review queue may use **Save draft** for their own private `ArticleEditWorkspace` draft, **Save** to apply their authorised article change, and **Revert to last published version** to discard their personal draft. Personal drafts are keyed by user/article/editor mode, so one manager cannot overwrite another manager's draft. If a separate shared Pending/Failed update appears, an explicitly saved manager/admin draft is preserved rather than silently deleted; the shared review workflow remains separate.

Reviewers use the explicit Manage Pending review flow. Submitted or rejected shared updates (`PENDING`/`FAILED`) may be reviewed; an owner's `NONE` draft is not exposed as shared review content. Selecting a dropdown status alone does not approve/reject. The workflow decision is applied only by the final review action.
