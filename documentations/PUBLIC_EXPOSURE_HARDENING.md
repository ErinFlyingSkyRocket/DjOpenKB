...
### 9.2 Persistent Existing-Article Edit/Review Checkpoints

Authorised Edit Article and Review Article pages use a private `ArticleEditWorkspace` keyed by user, article, and editor mode. Autosave updates only this checkpoint; it does not change the saved article, workflow status, reviewer queue, visibility, or SMTP state. Normal Save/Submit/Publish/Approve/Reject/Revert actions continue through the existing server-side permission and workflow validators.

In-site navigation is intercepted before redirecting and requires **Keep checkpoint and continue** or **Discard and continue**. Reset discards the private checkpoint and reloads the latest saved article/pending update. Browser unload restrictions are handled through delayed autosave, same-origin `sendBeacon`/keepalive attempts, and the browser-native warning while a newer change is unsaved.
...
Regression coverage is maintained in `kb/tests/articles/test_article_creation_workspace.py`, `kb/tests/articles/test_article_edit_workspace.py`, and `kb/tests/users/test_user_account_deletion_cleanup.py`.
