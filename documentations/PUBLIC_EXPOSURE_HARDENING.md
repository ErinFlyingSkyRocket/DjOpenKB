...
Permanent user deletion has a separate privacy cleanup path. A `pre_delete` user signal locks the account's private creation/edit workspaces and owned article rows. Saved Draft/Pending/Pending-failed articles are deleted with the account; any unpublished `pending_update_*` copy on preserved published knowledge is cleared; published knowledge remains with `SET_NULL` ownership and author snapshots. Checkpoint-specific private image audit rows are purged, authenticated/pending-MFA sessions are invalidated after commit, and private files are removed only after reference checks prove that no preserved article or another user's workspace still needs them. Disabled and inactive accounts do not trigger this path and retain their recoverable data.

### 9.2 Manual Existing-Article Editing and Shared Update Staging

Authorised Edit Article and Review Article pages use an owner-scoped `ArticleEditWorkspace`, but existing-article text is deliberately **manual-save only**. The workspace provides a secure editor context, temporary image ownership, and the server-owned approved-version baseline. Typing, changing review status, refreshing, or leaving the page does not autosave title/body/keywords/status. There is no existing-article **Reset edits** control or keep/discard autosave dialog.

A normal writer editing a published article uses the article's single staged update copy. **Save draft** writes proposed content to `pending_update_*` with `UpdateStatus.NONE`; it is excluded from Manage Pending but may be continued from normal Edit by the owner and authorised Managers/Admin Users. **Submit update for approval** changes that same staged copy to `PENDING`. If an already-submitted or rejected update is saved as draft again, it is retracted to `NONE`, its submission/review timestamps are cleared, and it leaves the reviewer queue until explicitly resubmitted.

Managers and Admin Users editing a published article outside the review queue do not receive a separate personal `ArticleEditWorkspace` draft. Their normal status selector operates on the shared staged copy while keeping the approved article live: Draft maps to `UpdateStatus.NONE`, Pending maps to `PENDING`, Pending failed maps to `FAILED`, and Published applies the current form to the live article and clears staging. A Manager/Admin who is also the owner retains the owner-side **Revert to last published version** action.

The explicit Review workflow is intentionally less destructive: reviewers can **Reset to user-submitted version** and then use **Save review action**, but they cannot discard the owner's submitted update with **Revert to last published version**. A forged review POST requesting that owner-side revert is rejected server-side.

Reviewers use the explicit Manage Pending review flow. Review mode is accepted only for a real Pending/Failed review state; ordinary published articles cannot be turned into review pages by forging `editor_mode=review`. Approver-only users receive the staged copy only through the Pending review flow. Selecting a dropdown status alone does not approve/reject. The workflow decision is applied only by the final review action. Existing submission snapshots are preserved when a Manager/Admin merely edits an already-Pending copy, so Reset to user-submitted version remains authoritative.

Approval/publication precedence uses only `ArticleEditWorkspace.article_approved_at_snapshot`, captured by the server when the workspace is created. Final operations compare that server-side baseline with the currently locked article. A hidden/form timestamp cannot override the check; a stale editor is rejected if a newer publication took precedence.

Uncommitted image uploads remain bound to the exact edit-workspace UUID and article ID. Final validation accepts only images already committed to that same article or uncommitted uploads owned by that exact workspace, preventing one article from borrowing another article's managed image. The display editor supports 25%, 50%, 75%, 100%, 125%, 150%, 175%, and 200% presets plus manual dimensions, while responsive rendering prevents article overflow. Video previews and published players use the same Admin-configured maximum width.

Keyword suggestion data is embedded with Django's JSON-safe script serialization rather than raw `json.dumps(... )|safe`, preventing article keywords from breaking out of the JSON data element. Static assets are configured for Nginx revalidation rather than a seven-day freshness lifetime, reducing mixed old-JavaScript/new-template behavior after deployment.

Regression coverage is maintained in `kb/tests/articles/test_article_creation_workspace.py`, `kb/tests/articles/test_article_edit_workspace.py`, `kb/tests/security/test_keyword_suggestion_json.py`, `kb/tests/security/test_nginx_admin_and_bulk_limits.py`, and `kb/tests/users/test_user_account_deletion_cleanup.py`.
...
### Structured image ownership hot path

Per-upload pending quota checks do not scan the full article repository or the OpenKB Markdown tree. Current uploads are attached to the authenticated user's New Article or existing-article workspace and quota usage is calculated from those structured ownership records, with only the exact recorded target article checked for legacy direct-editor rows. Per-file image validation, configured article-image count limits, and account-wide pending count/byte quotas remain enforced. Request-time image serving likewise uses structured article/snapshot/workspace image ownership rather than `body__icontains` fallbacks. Published-reader access is derived only from the live article's `image_assets`; an image referenced only by `pending_update_image_assets` or a review snapshot remains unpublished and requires owner/reviewer/manager authorisation. A file already used by the live published copy remains readable under that live article's normal visibility even if the staged copy also references it. Full Markdown/body scans remain maintenance-only for stray-file reconciliation and legacy/external edits.
