...
| Certificate validation | The SMTP backend uses normal system trust plus an optional mounted enterprise CA file. |
...
- The configured issuing CA chain is mounted under `ldap-certs/`.
...
1. Keep `EMAIL_NOTIFICATIONS_ENABLED=false` while the Exchange endpoint and CA chain are prepared.
